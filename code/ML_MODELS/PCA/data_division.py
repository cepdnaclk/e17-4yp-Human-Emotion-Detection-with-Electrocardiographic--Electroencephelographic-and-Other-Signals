...
DEVIDE_NUMBER = 1
Standardized = True
path = './STANDARDIZED_PCA_EEG/' if Standardized else './PCA_EEG/'
...
for i in range (23):
    for j in range (18):
        with open(f'{path}{i+1}/{j+1}/{i+1}_{j+1}_stimuli.csv', 'r') as csvfile:
            ...
            with open(output_directory + 'PCA_a.txt', 'a') as textfile_a:
                for line in final_a:
            ...
            with open(output_directory + 'PCA_b.txt', 'a') as textfile_b:
                for line in final_b:
